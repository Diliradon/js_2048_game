...
function resetGame() {
    ...
    for(let r = 0; r < SIZE_BOARD; r++){
        for(let c = 0; c < SIZE_BOARD; c++)if (board[r][c] === 0) {
...
function slideLeft() {
    for(let i = 0; i < SIZE_BOARD; i++)board[i] = slide(board[i]);
}
function slideRight() {
...
function getItemsColumsBoard(matrix) {
    const columsItems = [];
    for(let r = 0; r < matrix.length; r++)for(let c = 0; c < matrix.length; c++)columsItems.push(matrix[c][r]);
    return columsItems;
}
...
function slideUp() {
    ...
    for(let r = 0; r < SIZE_BOARD; r++)colums[r] = slide(colums[r]);
    const rows = getColumsBoard(getItemsColumsBoard(colums), SIZE_BOARD);
    for(let r = 0; r < SIZE_BOARD; r++)board[r] = rows[r];
}
function slideDown() {
    const colums = getColumsBoard(getItemsColumsBoard(board), SIZE_BOARD);
    for(let r = 0; r < SIZE_BOARD; r++){
